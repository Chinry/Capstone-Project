...
int main(void) {
    ...
    pwm_load = (SysCtlClockGet() / 64 / PWM_FREQUENCY) - 1;
    PWMGenConfigure(PWM1_BASE, PWM_GEN_0, PWM_GEN_MODE_DOWN);
    PWMGenPeriodSet(PWM1_BASE, PWM_GEN_0, pwm_load);

    PWMPulseWidthSet(PWM1_BASE, PWM_OUT_0, pwm_adjust * pwm_load / 1000);
...
#ifdef USE_PWM_TEST
        PWMOutputState(PWM1_BASE, PWM_OUT_0_BIT, true);
        PWMPulseWidthSet(PWM1_BASE, PWM_OUT_0, freq_f * pwm_load / 240);
#endif // USE_PWM_TEST
    }
    else {
        GPIOPinWrite(GPIO_PORTF_BASE, GPIO_PIN_1|GPIO_PIN_2|GPIO_PIN_3, 0x00);
